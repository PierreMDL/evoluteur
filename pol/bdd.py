from bdd import (
    garantir,
    obtenir_mot as base_obtenir_mot,
    obtenir_descendants as base_obtenir_descendants,
    obtenir_lexique as base_obtenir_lexique,
    Mot,
)

from .langue import POL, MotPOL

# Chaque fois que le module est initialisé, garantir que la langue est à jour
garantir(POL)


def obtenir_mot(mot: MotPOL) -> Mot:
    return base_obtenir_mot(mot=mot, langue=POL)


def obtenir_descendants(mot: MotPOL) -> list[dict]:
    return base_obtenir_descendants(mot=mot, langue=POL)


def obtenir_lexique() -> list[Mot]:
    return base_obtenir_lexique(langue=POL)
